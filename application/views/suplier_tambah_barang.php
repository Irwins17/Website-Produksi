...
<div class="wrapper bgded overlay gradient" style="background-image:url('<?=base_url()?>assets/images/demo/backgrounds/stp3.jpg');">
  <div id="breadcrumb" class="hoc clear"> 
    <h6 class="heading">Barang Dari Supplier/Customer</h6>
    <ul>
      <!-- <li><a href="#">Home</a></li>
...
<div class="wrapper row3">
  <main class="hoc container clear"> 
    <div class="sidebar one_quarter first"> 
      ...
      <ul>
          <li><a href="<?= base_url('Landing_page/suplier_view')?>"><b>Barang Dari Supplier</b></a></li>
          <li><a href="<?= base_url('Landing_page/suplier_user_view')?>">Tambah Suplier</a></li>
    ...
    <div class="content three_quarter"> 
        <form action="<?php echo base_url().'landing_page/proses_suplier_tambah_barang'; ?>" method="post">
			    <input type="hidden" name="id_suplier" id="id_suplier" value="" />
			<!-- <div class="mt-4 col-md-5">
			    <input class="form-control form-control-name" name="nama_suplier" id="nama_suplier" placeholder="Nama Supplier" type="text" value="" required>
			</div> -->
      <div class="mt-4 col-md-5">
      <select class="custom-select form-control" name="nama_suplier" required>
                <option value="" disabled selected>Nama Suplier</option>
                <?php foreach ($supplier as $row) : ?>
                  <option  class="form-control select2" value="<?php echo $row['nama_suplier'] ?>">  <?php echo $row['nama_suplier'] ?> </option>
                <?php endforeach; ?>
              </select>
            </div>

      <div class="mt-4 col-md-5">
      <select class="custom-select form-control" name="kode_material" required>
                <option value="" disabled selected>Nama Barang</option>
                <?php foreach ($ipb as $row) : ?>
                  <option  class="form-control select2" onchange="show_siswa(this.value)" value="<?php echo $row['kode_material'] ?>">  <?php echo $row['nama_material'] ?> </option>
                <?php endforeach; ?>
              </select>
            </div>
            <!-- <div class="mt-4 col-md-5">
				<input class="form-control form-control-name" name="kode_material" id="kode_material" placeholder="Nomer Dokumen" type="text" value="" required>
			</div>       -->
			<div class="mt-4 col-md-5">
          <label>Tanggal Pemasukan Barang dari Supplier :</label>
			    <input class="form-control form-control-name" name="tanggal" id="tanggal" placeholder="tanggal" type="date" required>
			</div>
      <div class="mt-4 col-md-5">
			    <input class="form-control form-control-name" name="barang_masuk" id="barang_masuk" placeholder="Jumlah" type="text" required>
			</div>
									
			<div class="plan-action mt-4 col-md-5">
				<button class="btn btn-info" type="submit" value="proses_suplier_tambah_barang" >Tambah Data</button>
			</div>
		</form>
